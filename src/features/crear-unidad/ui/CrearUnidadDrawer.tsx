...
import { useCrearUnidad } from '../hooks/useCrearUnidad';
import type { UbicacionNode } from '../hooks/useCrearUnidad';
...
interface Props {
  ...
  onCrear?: (nombre: string, codigo: string, unitType: UnitType, ubicacionId: string | null) => void;
  onGuardarBorrador?: (nombre: string, codigo: string, unitType: UnitType, ubicacionId: string | null) => void;
}

export function CrearUnidadDrawer({ open, unitType, onClose, isInitialState = false, onCrear, onGuardarBorrador }: Props) {
  const g = useCrearUnidad(isInitialState);

  const UnitIcon = unitType ? UNIT_ICON[unitType] : IconUsers;
  const title = unitType ? `Crear ${unitType.toLowerCase()}` : 'Crear unidad';

  // Adjust during render: reset form when unitType changes
  const [prevUnitType, setPrevUnitType] = useState(unitType);
  if (unitType !== prevUnitType) {
    setPrevUnitType(unitType);
    g.reset();
  }
  ...
  const handleCrear = () => {
    if (!g.canCreate || !unitType) return;
    g.createUnit(unitType);
    onCrear?.(g.nombre.trim(), g.codigo.trim(), unitType, g.selectedId);
    handleClose();
  };

  const handleGuardarBorrador = () => {
    if (!g.canCreate || !unitType) return;
    g.createUnit(unitType, 'Pendiente');
    onGuardarBorrador?.(g.nombre.trim(), g.codigo.trim(), unitType, g.selectedId);
    handleClose();
  };
