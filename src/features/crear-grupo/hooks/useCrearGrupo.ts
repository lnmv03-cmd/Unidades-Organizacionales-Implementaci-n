...
export function useCrearGrupo(isInitialState = false) {
  ...
  // Initial state: show only root node + locally created groups this session
  // Data state: show full store tree (or fallback root when store is empty)
  ...
  // Adjust during render: auto-expand nodes newly added to the tree
  ...
  const confirmAdd = () => {
    ...
    if (isInitialState) {
      // Ensure a root node exists in the store before adding children
      ...
      // Add to store so it appears in the structure view
      addNode({
        id:       newId,
      ...
      // Also track locally for display in the drawer tree
